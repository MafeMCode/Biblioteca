...
const BOOK_SIZE_MAX: number = 100;
const BOOK_SIZE_MIN: number = 15;

const Book = ({title, subtitle, author, color, orientation, design, width, imgURL}: BookProps) => {
            ...
            case "onDisplay":
                return(
                    ...
                    $color={appliedStyle(color)}>
                        ...
                        {title ? <>
                            <BookText title={title} subtitle={author}/>
                            <img src={imgURL} alt={title} width={100} height={100}/>
                            <p className="bookshelf__book-author">ISBN: {subtitle}</p>
                        </> : <></>}
                    </BookOnDisplay>
                );
            default:
